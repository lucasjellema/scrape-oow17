...
var sessionsTweetedAboutFileName = "sessionsTweetedAbout.json";
    ...
    for (var i = 0; i < sessions.length; i++) {
        if (sessions[i].files && sessions[i].files.length > 0) {
            ...
            if (!sessionsTweetedAbout[sessions[i].code]) {
                ...
                tweetCtr++;
                setTimeout(getMessenger(tweetMsg), tweetCtr * (45 + Math.floor(Math.random() * 180)) *1000);
            }//if
